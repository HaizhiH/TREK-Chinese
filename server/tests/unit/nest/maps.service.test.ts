...
const svc = new MapsService(new DatabaseService(db as never), photoCacheStub, {} as never, { getAmapConfig: () => ({ enabled: false }) } as never);
...
function settingsSvc(row?: { value: string }) {
  return new MapsService(makeSettingsDb(row).db, photoCacheStub, {} as never, { getAmapConfig: () => ({ enabled: false }) } as never);
}
...
describe('kill-switch settings reads', () => {
  ...
  it('queries the matching app_settings key', () => {
    ...
    const s = new MapsService(settingsDb, photoCacheStub, {} as never, { getAmapConfig: () => ({ enabled: false }) } as never);
...
describe('brandLogo', () => {
  ...
  const service = (): MapsService => new MapsService(new DatabaseService(db as never), photoCacheStub, {} as never, { getAmapConfig: () => ({ enabled: false }) } as never);
