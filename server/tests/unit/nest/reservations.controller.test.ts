...
describe('ReservationsController (parity with the legacy /api/trips/:tripId/reservations route)', () => {
  ...
  it('GET / returns reservations', () => {
    ...
    expect(new ReservationsController(svc, airtrailLink, {} as never).list(user, '5')).toEqual({ reservations: [{ id: 1 }] });
  });
  ...
  describe('POST /', () => {
    ...
    it('creates, runs budget sync, broadcasts accommodation + reservation, notifies', () => {
      ...
      expect(new ReservationsController(svc, airtrailLink, {} as never).create(user, '5', body, 'sock')).toEqual({ reservation: { id: 9 } });
    ...
    it('400s on a body id belonging to another trip, without writing', () => {
      ...
      expect(thrown(() => new ReservationsController(svc, airtrailLink, {} as never).create(user, '5', body)))
    ...
    it('answers a foreign id with the older message when it is both', () => {
      ...
      expect(thrown(() => new ReservationsController(svc, airtrailLink, {} as never).create(user, '5', { title: 'Hotel', place_id: 4711 })))
  ...
  describe('PUT /positions', () => {
    ...
    it('updates positions and broadcasts', () => {
      ...
      expect(new ReservationsController(svc, airtrailLink, {} as never).updatePositions(user, '5', { positions, day_id: 3 }, 'sock')).toEqual({ success: true });
  ...
  describe('PUT /:id', () => {
    it('404 when the reservation is missing', () => {
      ...
      expect(thrown(() => new ReservationsController(svc, airtrailLink, {} as never).update(user, '5', '9', { title: 'X' }))).toEqual({ status: 404, body: { error: 'Reservation not found' } });
    });
    ...
    it('updates, syncs budget with current fallbacks, broadcasts + notifies', () => {
      ...
      new ReservationsController(svc, airtrailLink, {} as never).update(user, '5', '9', { create_budget_entry: { total_price: 50 } }, 'sock');
    ...
    it('400s on a body id belonging to another trip, without writing', () => {
      ...
      expect(thrown(() => new ReservationsController(svc, airtrailLink, {} as never).update(user, '5', '9', { day_id: 1, place_id: 2 })))
  ...
  describe('PUT /:id/travelers', () => {
    ...
    it('404 when the reservation is off-trip / missing', () => {
      ...
      expect(thrown(() => new ReservationsController(svc, airtrailLink, {} as never).updateTravelers(user, '5', '9', { user_ids: [1] }))).toEqual({ status: 404, body: { error: 'Reservation not found' } });
    });
    ...
    it('assigns travelers, broadcasts, and returns { travelers, reservation }', () => {
      ...
      expect(new ReservationsController(svc, airtrailLink, {} as never).updateTravelers(user, '5', '9', { user_ids: [2] }, 'sock')).toEqual({ travelers, reservation });
  ...
  describe('DELETE /:id', () => {
    it('404 when nothing deleted', () => {
      ...
      expect(thrown(() => new ReservationsController(svc, airtrailLink, {} as never).remove(user, '5', '9'))).toEqual({ status: 404, body: { error: 'Reservation not found' } });
    });
    ...
    it('broadcasts the accommodation + budget cascade then reservation:deleted', () => {
      ...
      expect(new ReservationsController(svc, airtrailLink, {} as never).remove(user, '5', '9', 'sock')).toEqual({ success: true });
