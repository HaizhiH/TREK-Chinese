import { boundsCenter, isInChinaMainland, shouldPreferAmap, type GeoPoint, type MapsRouteResult, type RouteProfile } from '@trek/shared';
...
interface PoiSearchResult {
   ...
   */
  source: 'openstreetmap' | 'trek-places' | 'amap';
  truncated: boolean;
...
export class MapsService {
  ...
  async detailsForProvider(
    ...
    if (provider === 'openstreetmap') return this.getPlaceDetails(userId, placeId, lang) as Promise<MapsPlaceDetailsResult>;
    if (provider === 'google') return this.getPlaceDetails(userId, placeId, lang) as Promise<MapsPlaceDetailsResult>;
  ...
  async reverse(lat: string, lng: string, lang?: string): Promise<MapsReverseResult> {
    ...
    const result = (await this.reverseGeocode(lat, lng, lang)) as MapsReverseResult;
  ...
  async pois(
    ...
    if (this.amapConfig?.getAmapConfig().enabled && isInChinaMainland(boundsCenter(bbox))) {
      try {
        const result = await this.amap.pois(category, bbox);
        return { ...result, pois: result.pois.map(poi => ({ ...poi, osm_id: poi.osm_id || `amap:${poi.provider_place_id}`, poi_type: category, opening_hours: null, cuisine: null, brand: null, brand_wikidata: null, charging: null })) };
      } catch { /* retain the index/Overpass fallback */ }
    }
