...
export class CollectionsService {
  ...
  `,
      ...
      this.db.run('UPDATE collection_places SET geo_provider = ?, provider_place_id = ? WHERE id = ?',
        body.geo_provider ?? (body.google_place_id ? 'google' : body.osm_id ? 'openstreetmap' : null),
        body.provider_place_id ?? body.google_place_id ?? body.osm_id ?? null, id);
  ...
  `);
    ...
    this.db.transaction(() => {
      for (const placeId of placeIds) {
        ...
        this.db.run('UPDATE collection_places SET geo_provider = ?, provider_place_id = ? WHERE id = ?',
          (p.geo_provider as string | null) ?? null, (p.provider_place_id as string | null) ?? null, Number(res.lastInsertRowid));
  ...
  copyToTrip(userId: number, body: CollectionCopyToTripRequest): { copied: number; skipped: { id: number; name: string }[] } {
    ...
    const sources: Array<{ id: number; name: string; description: string | null; lat: number | null; lng: number | null;
      ...
      osm_id: string | null; geo_provider: string | null; provider_place_id: string | null; website: string | null; phone: string | null; collection_id: number }> = [];
  ...
  `);
    ...
    this.db.transaction(() => {
      for (const s of sources) {
        ...
        this.db.run('UPDATE places SET geo_provider = ?, provider_place_id = ? WHERE id = ?', s.geo_provider, s.provider_place_id, newPlaceId);
