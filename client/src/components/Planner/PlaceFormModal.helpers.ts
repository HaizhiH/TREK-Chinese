...
export function mergeResult(
  ...
  // Provider identities are never user text; every pick replaces all of them.
  next.google_place_id = (result.google_place_id ?? null) as string | null
  next.google_ftid = (result.google_ftid ?? null) as string | null
  next.osm_id = (result.osm_id ?? null) as string | null
  next.amap_poi_id = (result.amap_poi_id ?? undefined) as string | undefined
  next.geo_provider = (result.provider ?? result.geo_provider ?? null) as PlaceFormData['geo_provider']
  next.provider_place_id = (result.provider_place_id ?? null) as string | null
  return next
}
