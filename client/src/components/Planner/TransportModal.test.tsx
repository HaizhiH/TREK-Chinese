...
describe('TransportModal', () => {
  ...
  it('ignores a pending 12306 response after changing the train number', async () => {
    let release!: () => void;
    const waiting = new Promise<void>(resolve => { release = resolve; });
    let started = false;
    server.use(http.get('/api/trips/1/reservations/train/12306', async () => {
      started = true;
      await waiting;
      return HttpResponse.json({ stops: [{ sequence: 1, name: 'Stale station', departureTime: '06:30' }] });
    }));
    ...
    const input = screen.getAllByPlaceholderText('G1')[0];
    await userEvent.type(input, 'G1');
    await userEvent.click(screen.getByRole('button', { name: /Find train/i }));
    await waitFor(() => expect(started).toBe(true));
    await userEvent.clear(input);
    await userEvent.type(input, 'G2');
    release();
    await waitFor(() => expect(screen.getByRole('button', { name: /Find train/i })).toBeEnabled());
    expect(screen.queryByText('Stale station')).not.toBeInTheDocument();
  });
