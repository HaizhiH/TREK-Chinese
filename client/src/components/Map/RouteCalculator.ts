...
async function routeData(
  url: string, signal: AbortSignal | undefined, surface: 'route' | 'segments' | 'legs',
  profile: 'driving' | 'walking' | 'cycling', waypoints: Waypoint[],
) {
  if (waypoints.every(isInChinaMainland)) {
    const route = await mapsApi.route(waypoints, profile, signal)
    return { code: 'Ok', routes: [{ ...route, geometry: {
      coordinates: route.geometry.map(({ lat, lng }) => [lng, lat]),
    } }] }
  }
  const response = await routedFetch(url, signal, surface, profile, waypoints)
...
export async function calculateRoute(
  ...
  const data = await routeData(url, signal, 'route', profile, waypoints)
...
export async function calculateSegments(
  ...
  const data = await routeData(url, signal, 'segments', 'driving', waypoints)
...
export async function calculateRouteWithLegs(
  ...
  const data = await routeData(url, signal, 'legs', osrmProfile, waypoints)
  if (data.code !== 'Ok' || !data.routes?.[0]) throw new Error('No route found')
